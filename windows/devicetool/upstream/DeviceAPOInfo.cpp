...
// Isotone modification: VoicemeeterAPOInfo is not vendored.
...
bool DeviceAPOInfo::checkAPORegistration(bool fix)
{
	...
	if (!RegistryHelper::keyExists(apoRegistrationKeyPath L"\\" + RegistryHelper::getGuidString(ISOAPO_PRE_MIX_GUID))
		|| !RegistryHelper::keyExists(apoRegistrationKeyPath L"\\" + RegistryHelper::getGuidString(ISOAPO_POST_MIX_GUID))
		|| !RegistryHelper::keyExists(clsidKeyPath L"\\" + RegistryHelper::getGuidString(ISOAPO_PRE_MIX_GUID))
		|| !RegistryHelper::keyExists(clsidKeyPath L"\\" + RegistryHelper::getGuidString(ISOAPO_POST_MIX_GUID)))
	{
		result = false;
		...
		if (fix)
		{
			...
			if (GetModuleFileNameW(NULL, path, MAX_PATH) != 0)
			{
				...
				wstring params = wstring(L"/s \"") + path + L"\\IsoAPO.dll\"";   // Isotone modification
	...
	else
	{
		for (int i = 0; i < allGuidValueNameCount; i++)
		{
			if (RegistryHelper::valueExists(keyPath + L"\\FxProperties", allGuidValueNames[i]))
			{
				...
				if (originalApoGuid == RegistryHelper::getGuidString(ISOAPO_PRE_MIX_GUID) || originalApoGuid == RegistryHelper::getGuidString(ISOAPO_POST_MIX_GUID))
		...
		for (int i = 0; i < allGuidValueNameCount; i++)
		{
			...
			if (RegistryHelper::valueExists(keyPath + L"\\FxProperties", allGuidValueNames[i]))
			{
				...
				{
					if (apoGuid == ISOAPO_PRE_MIX_GUID || apoGuid == ISOAPO_POST_MIX_GUID)
					{
						foundAt[i] = true;
						found = true;
					}
...
void DeviceAPOInfo::install()
{
	...
	if (!RegistryHelper::keyExists(keyPath + L"\\FxProperties"))
	{
		...
		RegistryHelper::writeValue(keyPath + L"\\FxProperties", fxTitleValueName, L"IsoAPO");   // Isotone modification
	...
	if (selectedInstallState.installMode == INSTALL_LFX_GFX)
	{
		if (selectedInstallState.installPreMix)
			RegistryHelper::writeValue(keyPath + L"\\FxProperties", lfxGuidValueName, RegistryHelper::getGuidString(ISOAPO_PRE_MIX_GUID));
		if (selectedInstallState.installPostMix && !input)
			RegistryHelper::writeValue(keyPath + L"\\FxProperties", gfxGuidValueName, RegistryHelper::getGuidString(ISOAPO_POST_MIX_GUID));
	...
	else if (selectedInstallState.installMode == INSTALL_SFX_MFX)
	{
		...
		if (selectedInstallState.installPreMix)
		{
			RegistryHelper::writeValue(keyPath + L"\\FxProperties", sfxGuidValueName, RegistryHelper::getGuidString(ISOAPO_PRE_MIX_GUID));
		...
		if (selectedInstallState.installPostMix && !input)
		{
			RegistryHelper::writeValue(keyPath + L"\\FxProperties", mfxGuidValueName, RegistryHelper::getGuidString(ISOAPO_POST_MIX_GUID));
	...
	else if (selectedInstallState.installMode == INSTALL_SFX_EFX)
	{
		...
		if (selectedInstallState.installPostMix && !input)
		{
			RegistryHelper::writeValue(keyPath + L"\\FxProperties", efxGuidValueName, RegistryHelper::getGuidString(ISOAPO_POST_MIX_GUID));
