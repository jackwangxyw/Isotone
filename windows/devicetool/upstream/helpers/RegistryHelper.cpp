...
wstring RegistryHelper::readValue(wstring key, wstring valuename)
{
	...
	if (dryRun && dryRun->readValue(key, valuename, &result))   // Isotone modification
		return result;
...
bool RegistryHelper::keyExists(wstring key)
{
	...
	if (dryRun && dryRun->keyExists(key, &result))   // Isotone modification
		return result;
...
{
	bool overridden;   // Isotone modification
	if (dryRun && dryRun->valueExists(key, valuename, &overridden))
		return overridden;
...
{
	bool overridden;   // Isotone modification
	if (dryRun && dryRun->keyEmpty(key, &overridden))
		return overridden;
