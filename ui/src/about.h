...
// (diagnostics.h says where each comes from), and Copy diagnostics. On Linux the
// engine rows are the daemon and PipeWire, and there is no protected audio. Read on
...
class About : public QObject {
    ...
    // Linux: "Running · <output it feeds>" or "Not running", and PipeWire's version.
    Q_PROPERTY(QString daemon READ daemon NOTIFY changed)
    Q_PROPERTY(QString pipewire READ pipewire NOTIFY changed)
...
public:
    ...
    QString daemon() const { return daemon_; }
    QString pipewire() const { return pipewire_; }
